...
class AjadwalController extends Controller
{
    ...
    function cek()
    {
        ...
        $list_senin = $data['list_senin'] = jam_jadwal::where('hari', 'senin')->orderBy('jam_ke', 'asc')->get();
            foreach($list_senin as $senin ){
                $senin->mapel = Jadwal::where('id_kelas',request('kelas'))->where('id_jam',$senin->id)->first();
            }
